...
def calc_damage(attacker: BattlePet, defender: BattlePet, skill: BattleSkill, typechart: dict,
                ...
                extra_power_flat: float = 0.0, state=None, element_override=None) -> dict:
    if skill.power is None or skill.category not in (0, 1):
        return {"damage": 0, "display_power": 0, "type_multiplier": 1.0, "stab": 1.0}

    from . import features
    elem = element_override if element_override is not None else skill.element
    attack_key = "atk" if skill.category == 0 else "spatk"
    defense_key = "def" if skill.category == 0 else "spdef"
    atk_mult = buffs.get_stat_multiplier(attacker, attack_key) * features.modify_stat(attacker, attack_key, 1.0)
    def_mult = buffs.get_stat_multiplier(defender, defense_key) * features.modify_stat(defender, defense_key, 1.0)
    if state is not None:
        atk_mult *= features.modify_stat_state(state, attacker.side, attack_key, 1.0)
        def_mult *= features.modify_stat_state(state, defender.side, defense_key, 1.0)
    attack_stat = attacker.stats[attack_key] * atk_mult
    defense_stat = defender.stats[defense_key] * def_mult
    defender_elements = getattr(defender, "attributes", [])
    stab = stab_multiplier(attacker, skill) if elem == skill.element else (1.25 if elem in getattr(attacker, "attributes", []) else 1.0)
    type_mult = type_multiplier(elem, defender_elements, typechart)
